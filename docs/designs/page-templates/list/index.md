...
> 前端分页不适合数据量大的时候，因为首次加载用户等待时间长，但是一旦加载完成，本地用户操作无时延体验会比较好，这种方式后端接口调用少，业务都放在了前端；后端分页的优势是首屏加载快，数据按需请求，没有网络请求浪费，这种方式后端提供的接口多，业务都放在了后端，前端只负责展示；排序分组过滤都支持前后端分页 2 种模式，antd table 和 ag-grid 都支持，不过 antd-table 默认不支持虚拟滚动，单页大数据量展示时会卡顿，可以用分页解决，现在有一种场景例外就是树形数据，因为展开后的数量可能非常大；后端分页模式下的选择，一般加上过滤条件之后数据就很少了，并且用户可以逐步放大 pageSize，或者把搜索条件带上在后端执行“全部操作”
...
行选择和选中后的批量操作是绑定行为，所以不会单独出现行选择器

通过行最左侧 checkbox 控件对行进行选择操作，用户可以依次点击达到批量选择行的效果，也可以点击表头最左侧的 checkbox 进行`当前页的`的批量快捷选择，选中状态后继续点击表示立即`清空当前页`，可能的侧边下拉框的菜单中也包含了其他一些快捷操作：`选择全部`，`反选当前页`，`清空全部`

表格列数量为空的时候，比如异步初始化表头在未返回数据前，此时，默认不显示行操作单独列

切换分页的时候，会保留选中状态，也就是说用户在当前页选中的行，进入下一页再回来时依旧展示
